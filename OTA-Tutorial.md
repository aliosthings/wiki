...
阿里云平台主要包括[物联网平台](http://iot.console.aliyun.com)和[智能生活开放平台](https://living.aliyun.com)两大平台；针对智能开放平台目前只推出了基础的整包升级服务，针对物联网平台推出了整包升级(提供基础整包固件升级)，差分升级 (提供差分增量包固件升级)，安全升级（提供安全数字签名验证升级）服务。默认选择的是整包升级，开发者或用户可以根据应用场景选择使用差分升级和安全升级。
